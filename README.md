...
1. Open Intellij (if you are not in the welcome screen, click `hinlok.storage` > `Close Project` to close the existing
   project first)
...
3. After that, locate the `src/main/java/Duke.java` hinlok.storage, right-click it, and choose `Run Duke.main()` (if the
